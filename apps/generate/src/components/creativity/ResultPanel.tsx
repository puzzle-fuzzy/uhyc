      ...
      {isActive ? (
        ...
        <>
          ...
          <div className="crea-result__actions">
            <button
              type="button"
              className="uhyc-btn uhyc-btn--accent uhyc-btn--small"
              onClick={() => downloadText(content, `creativity-${stepName}.txt`)}
            >
              下载 TXT
            </button>
            {stepIdx === 0 && srtContent && (
              <button
                type="button"
                className="uhyc-btn uhyc-btn--ghost uhyc-btn--small"
                onClick={() => downloadText(srtContent, `creativity-${stepName}.srt`)}
              >
                下载 SRT
              </button>
            )}
        ...
        <div key={task.id} className="crea-task">
          <div className="crea-task__head">
            <span className="crea-task__title">
              {task.status === 'SUCCEEDED' ? '(^_^)' : task.status === 'FAILED' ? '(>_<)' : '(◕‿◕)'} 处理任务
            </span>
          ...
          <ResultCard title="语音识别" content={task.asrResult?.text ?? null} stepIdx={0} task={task} srtContent={task.asrResult?.srt} />
          <ResultCard title="视频理解 — 剧本" content={task.scriptResult} stepIdx={1} task={task} />
          <ResultCard title="合并脚本" content={task.mergedResult} stepIdx={2} task={task} />
