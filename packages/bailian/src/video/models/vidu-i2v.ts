...
// Vidu 图生视频（基于首帧）
//
// 对应文档: docs/bailian/Vidu/Vidu-图生视频-基于首帧.md
// API 端点: POST /api/v1/services/aigc/video-generation/video-synthesis
//
// Q3 Pro / Q3 Turbo / Q2 Pro-Fast / Q2 Pro / Q2 Turbo 五个模型变体。
// Q3 系列支持 audio，Q2 系列不支持。
// media 字段：1 张图片（首帧）。
// ---------------------------------------------------------------------------

const BASE_FIELDS_I2V = [
  {
    key: 'prompt',
    label: '文本提示词',
    type: 'text' as const,
    ...
    maxLength: 5000,
    description: '描述期望生成的视频内容（可选）。支持中英文，不超过5000个字符',
  },
  {
    key: 'media',
    label: '首帧图片',
    ...
    mediaSlots: [
      { type: 'image' as const, label: '首帧图片', accept: 'image/*', maxCount: 1, maxSizeMB: 50 },
    ],
  },
  {
    key: 'resolution',
...
export const viduQ3ProI2v: ModelDefinition<VideoSubCategory> = {
  ...
  fields: [
    ...
    AUDIO_FIELD,
    BASE_FIELDS_I2V[2],
    { ...BASE_FIELDS_I2V[3], min: 1, max: 16 },
    BASE_FIELDS_I2V[4],
    BASE_FIELDS_I2V[5],
  ],
}
...
export const viduQ2ProFastI2v: ModelDefinition<VideoSubCategory> = {
  ...
  pricing: {
    ...
    region: 'cn-beijing',
    tiers: [
      { condition: { resolution: '720P' }, price: 0.1 },
      { condition: { resolution: '1080P' }, price: 0.2 },
    ],
  },
  fields: [
    BASE_FIELDS_I2V[0],
...
export const viduQ2ProI2v: ModelDefinition<VideoSubCategory> = {
  ...
  fields: [
    ...
    BASE_FIELDS_I2V[1],
    BASE_FIELDS_I2V[2],
    { ...BASE_FIELDS_I2V[3], min: 1, max: 10 },
    BASE_FIELDS_I2V[4],
    BASE_FIELDS_I2V[5],
  ],
}
