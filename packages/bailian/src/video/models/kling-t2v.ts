...
const BASE_FIELDS_KLING_T2V = [
  ...
  },
  {
    ...
    group: 'input' as const,
    defaultValue: false,
    description: '开启后启用多镜头生成。智能模式由模型自动规划镜头；自定义模式支持独立控制每个片段的提示词和时长',
  },
  {
    key: 'shot_type',
    label: '分镜模式',
    ...
    options: [
      { label: '智能分镜（模型自动规划）', value: 'intelligence' },
      { label: '自定义分镜（手动设置每镜）', value: 'customize' },
    ],
    description: '仅当智能分镜开启时生效。intelligence=模型自动规划，customize=手动编写分镜脚本',
  },
  {
    key: 'multi_prompt',
    label: '分镜脚本',
    type: 'shot-list' as const,
    group: 'input' as const,
    min: 1,
    max: 15,
    description: '自定义每个分镜片段的提示词与时长。1~6 个片段，每个时长 1~15 秒。仅当分镜模式为"自定义"时生效',
  },
  {
    key: 'watermark',
    label: '添加水印',
