...
| --- | --- | --- | --- | --- | --- | --- |
| **（Token数）** |   |   | **（每百万 Token）** |   |
| qwen-plus-character | 32,768 | 32,768 | 4,096 | 3.747元 | 10.492元 | 无免费额度 |
| qwen-flash-character | 8,192 | 8,192 | 4,096 | 0.375元 | 2.998元 |
...
角色扮演模型的输入与输出参数请参见[文本生成](https://help.aliyun.com/zh/model-studio/qwen-api-reference/)。

## **前提条件**

已[获取API Key](https://help.aliyun.com/zh/model-studio/get-api-key)并[配置API Key到环境变量](https://help.aliyun.com/zh/model-studio/configure-api-key-through-environment-variables)。如果通过 OpenAI SDK 或 DashScope SDK 进行调用，需要[安装SDK](https://help.aliyun.com/zh/model-studio/install-sdk)。
...
代码示例中的URL以北京地域为例，如在新加坡地域使用需要替换为`https://{WorkspaceId}.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1`。模型名称需要替换为`qwen-plus-character-ja`。System、Assistant和User Message也可做相应替换。
...
代码示例中的URL以北京地域为例，如在新加坡地域使用需要替换为`https://{WorkspaceId}.ap-southeast-1.maas.aliyuncs.com/api/v1`。模型名称需要替换为`qwen-plus-character-ja`。System、Assistant和User Message也可做相应替换。
...
| **参数名** | **类型** | **是否必填** | **说明** |
| --- | --- | --- | --- |
| `enable_realtime` | boolean | 否   | 是否启用真实时间，默认值为 `false`。启用后模型可感知当前日期和时间。 |
...
// 定义角色人设（原System Message内容迁移到profile）
const profile = "你是江让，男性，一个围棋天才，拿过很多围棋的奖项。你现在在读高中，是高中校草，用户是你的班长。一开始你看用户在奶茶店打工，你很好奇，后来慢慢喜欢上用户了。\n\n你的性格特点：\n\n热情，聪明，顽皮\n\n你的行事风格：\n\n机智，果断\n\n你的语言特点：\n\n说话幽默，爱开玩笑\n\n你可以将动作、神情语气、心理活动、故事背景放在（）中来表示，为对话提供补充信息。";
...
> 创建知识库计费请参见[知识库计费说明](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base)。
...
| **参数名** | **类型** | **是否必填** | **说明** |
| --- | --- | --- | --- |
| `vector_store_ids` | array | 否   | 知识库 ID 列表，string 数组类型。当前支持传入最多 10 个知识库 ID。知识库 ID 可在[百炼控制台](https://bailian.console.aliyun.com/cn-beijing#/knowledge-base)的知识库详情页查看。 |
...
> 联网搜索费用请参见[计费说明](https://help.aliyun.com/zh/model-studio/web-search#92ce83df3a599)。

#### **开启方式**

通过 `enable_search: true` 参数启用联网搜索功能，参见[联网搜索](https://help.aliyun.com/zh/model-studio/web-search) 。

| **参数名** | **类型** | **说明** |
| --- | --- | --- |
| `enable_search` | boolean | 是否启用联网搜索，默认值为 false。 |
...
支持通过`search_options`设置的联网搜索能力：

-   [设置搜索量级策略](https://help.aliyun.com/zh/model-studio/web-search#5a7d30420fc5k)
    
    > 仅支持turbo、max策略。
    
-   [强制联网搜索](https://help.aliyun.com/zh/model-studio/web-search#6f83c89f7dw0v)
    
-   [开启垂域搜索](https://help.aliyun.com/zh/model-studio/web-search#b12beb35b0hy4)
    
-   [设置搜索时效性](https://help.aliyun.com/zh/model-studio/web-search#4f84e80e30i5a)
    
-   [联网搜索](https://help.aliyun.com/zh/model-studio/web-search#assigned-site-list-title)
    
-   [通过自然语言控制检索范围](https://help.aliyun.com/zh/model-studio/web-search#f58e95a5cam6f)
...
大模型的输入输出中可能包含敏感或高风险内容，例如涉黄、涉政和广告等。大模型自有的合规检查机制通常能够提供有效的内容安全保障。此外，阿里云百炼支持接入内容安全服务，进一步识别输入输出内容的违规信息，保障输入输出内容的安全与合规性。如果调整相关的内容审核尺度，请参考[输⼊输出 AI 安全护栏](https://help.aliyun.com/zh/model-studio/content-security)。
