...
| ##### 请求体（Request Body） |
| **model** `*string*` **（必选）** 模型名称。 可选值： - `happyhorse-1.1-i2v` - `happyhorse-1.0-i2v` |
